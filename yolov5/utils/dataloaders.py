...
class LoadImagesAndLabels(Dataset):
    ...
    def check_cache_ram(self, safety_margin=0.1, prefix=""):
        ...
        for _ in range(n):
            im = tiff.imread(random.choice(self.im_files))  # sample image
    ...
    def load_image(self, i):
        ...
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                ...
                im = tiff.imread(f)  # Change to TIFF
                assert im is not None, f"Image Not Found {f}"
            h0, w0 = im.shape[:2]  # orig hw
            r = self.img_size / max(h0, w0)  # ratio
            if r != 1:  # if sizes are not equal
                # Hard Coded the interpolation to nearest neighbor
                interp = cv2.INTER_NEAREST if (self.augment or r > 1) else cv2.INTER_NEAREST
                im = cv2.resize(im, (math.ceil(w0 * r), math.ceil(h0 * r)), interpolation=cv2.INTER_NEAREST)
    ...
    def cache_images_to_disk(self, i):
        ...
        f = self.npy_files[i]
        if not f.exists():
            np.save(f.as_posix(), tiff.imread(self.im_files[i]))
...
def extract_boxes(path=DATASETS_DIR / "coco128"):
    ...
    for im_file in tqdm(files, total=n):
        if im_file.suffix[1:] in IMG_FORMATS:
            # image
            im = tiff.imread(str(im_file))[..., ::-1]  # BGR to RGB
            h, w = im.shape[:2]
...
def verify_image_label(args):
    ...
    try:
        ...
        # Checking if the tiff image have atleast 3 channels
        if im.shape[2] < 3 :
            raise ValueError(f"Expected image with more than 3 channels, but got {im.shape}")
    ...
    def __getitem__(self, i):
        ...
        if self.cache_ram and im is None:
            ...
            if not fn.exists():  # load npy
                np.save(fn.as_posix(), tiff.imread(f))
            im = np.load(fn)
        else:  # read image
