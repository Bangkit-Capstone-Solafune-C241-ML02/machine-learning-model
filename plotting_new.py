...
class Annotator:
    ...
    def box_label(self, box, label="", color=(128, 128, 128), txt_color=(255, 255, 255), rotated=False):
        ...
        if self.pil or not is_ascii(label):
            ...
            # Ensure 'self.im' is a PyTorch tensor
            if isinstance(self.im, torch.Tensor) :
                bgr_image = self.im[2:5].permute(1,2,0).contiguous().cpu().numpy()
                bgr_image = (bgr_image * 255).astype(np.uint8)
            elif isinstance(self.im, np.ndarray):
                bgr_image = self.im[:, :, 2:5].copy()
                bgr_image = (bgr_image * 255).astype(np.uint8)[:, :, ::-1]

            #Ensure the image is contigious
            bgr_image = np.ascontiguousarray(bgr_image)

            if rotated:
                p1 = [int(b) for b in box[0]]
                # NOTE: cv2-version polylines needs np.asarray type.
                cv2.polylines(bgr_image, [np.asarray(box, dtype=int)], True, color, self.lw)
            else:
                p1, p2 = (int(box[0]), int(box[1])), (int(box[2]), int(box[3]))
                cv2.rectangle(bgr_image, p1, p2, color, thickness=self.lw, lineType=cv2.LINE_AA)
            if label:
                w, h = cv2.getTextSize(label, 0, fontScale=self.sf, thickness=self.tf)[0]  # text width, height
                outside = p1[1] - h >= 3
                p2 = p1[0] + w, p1[1] - h - 3 if outside else p1[1] + h + 3
                cv2.rectangle(bgr_image, p1, p2, color, -1, cv2.LINE_AA)  # filled
                cv2.putText(
                    bgr_image,
                    label,
            ...
            if isinstance(self.im, torch.Tensor):
                self.im[2:5] = torch.tensor(bgr_image[:, :, ::-1]).permute(2, 0, 1).to(self.im.device) / 255.0
            elif isinstance(self.im, np.ndarray):
                self.im[:, :, 2:5] = bgr_image[:, :, ::-1] / 255.0
    ...
    def masks(self, masks, colors, im_gpu, alpha=0.5, retina_masks=True):
        ...
        combined_mask = masks.squeeze(-1).sum(dim=0) > 0
        combined_mask = combined_mask.cpu().numpy().astype(np.uint8) * 255
        
        plt.imsave('/datadisk2/c241_ml02/workspace/combined_mask.png', combined_mask, cmap='gray')
        

	# Extract the BGR channels from the 20 channel image
        bgr_channels = im_gpu[1:4]
        bgr_channels = bgr_channels.flip(dims=[0])  # flip channel
        bgr_channels = bgr_channels.permute(1, 2, 0).contiguous()  # shape(h,w,3)
        bgr_channels = bgr_channels * inv_alpha_masks[-1] + mcs
        bgr_mask = bgr_channels * 255

        im_mask_np = bgr_mask.cpu().numpy()

        im_copy = self.im.copy()

        if retina_masks:
            im_copy[:, :, 1:4] = im_mask_np
        else :
            scaled_bgr = ops.scale_image(im_mask_np, self.im.shape[:2])
            im_copy[:, :, 1:4] = scaled_bgr
